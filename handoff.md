...
- **3 fixes de robustez achados pela carga real** (`01b1a2a`, `82fad1e`, `b864e81`):
  ...
  3. NuExtract flubba ~1/3 das obs de catálogo deterministicamente → fallback de decisão via phi4-mini + JSON schema; invariante refinado em `d5acbef` (rejeição sem justificativa OU aprovação sem fato = inconclusivo; `_parse_verdict` centraliza o parse). Obs de catálogo rejeitadas por variância do juiz foram redepositadas via sweep idempotente (`metadata.redeposit_of`: #18→#150, #17→#151, #14→#152, #22→#153). Causa da variância corrigida no prompt do juiz (`41f43ec`): as regras não diziam que dado de catálogo/preço/spec é fato válido — agora dizem.
...
- **TRUSTED PATH (07-07, `4ac5e17`) — decisão arquitetural**: o juiz phi4-mini rejeitava 20–30% do catálogo como "subjetivo" mesmo após fix de prompt (`41f43ec`) — loteria inaceitável sobre dado que é verdade por construção. `--catalog`/`--rank-csv` agora promovem direto (embed→dedup→verify, confiança 1.0, ~9s/obs vs ~150s no juiz); dedup por **slug** com fallback sim ≥0.995 (produtos irmãos cruzam 0.95 — 2 corroborações falsas aconteceram antes do fix: fatos #22/#33 com contador +1 espúrio). `--promote-pending` recupera obs confiáveis paradas. Catálogo 30/30 na memória. Filter fica só com obs do Scout (o trabalho real do juiz). Resíduo: ~11 produtos com fato duplicado (versão do juiz com typo + versão trusted) — dreaming consolida ≥0.96.
- **JUIZ PROMOVIDO A 7B (07-07, `f435863`+`e1337fb`) — achado central**: phi4-mini 3.8B não sustenta padrão epistêmico nuançado (3 iterações de prompt, ~90% falso negativo em conteúdo de negócio, virou "atribuição obrigatória" em critério de admissão, aprovou fato nonsense). qwen2.5:7b-instruct na VPS resolveu de primeira (fato institucional com atribuição "Segundo a documentação da ROI Labs..."). Env: `FILTER_REASONING_MODEL`+`FILTER_OLLAMA_URL`. Fixes de apoio: embed timeout 180s + guarda de dimensão (fallback nomic 768d → descartado, nunca 422 no verify) + clamp confiança 0–100→0–1. Custo ~3–4min/obs (swap 7B↔embed na VPS, MAX_LOADED=1).
- **Pendência**: deixar `python -m agents.filter --reprocess-stuck --loop 30 --limit 5` (com FILTER_REASONING_MODEL=qwen2.5:7b-instruct e FILTER_OLLAMA_URL=$OLLAMA_VPS_URL) rodando até zerar a fila; typos tipo "Goiçana" nos clean_facts (gap 5) — candidato: passe de revisão no dreaming ou prompt do filter.
